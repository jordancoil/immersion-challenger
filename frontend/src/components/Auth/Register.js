...
import { validateRegistration } from "../../helpers/auth.helper";
...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    const validationErrors = validateRegistration(email, password, passwordConfirmation)

    if (validationErrors.length > 0) {
      setFormError(validationErrors.map(error => <div>{error}</div>))
      return
    }
